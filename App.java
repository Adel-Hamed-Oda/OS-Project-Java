...
public class App {
    ...
    public static Queue waitingQueue = new Queue();
    public static Queue jopPool = new Queue();

    
    final private static ArrayList<Integer> arrival_times = new ArrayList<Integer>(Arrays.asList(0, 1, 4));
    //How to compute burst times for each process?
    ...
    public static ArrayList<Process> convertReadyQueueToProcesses() {
        ...
        ArrayList<Integer> readyQueueArr = readyQueue.getArr();
        for (int i = 0; i < readyQueueArr.size(); i++) {
            int p_id = readyQueueArr.get(i);
            // Assuming you have a way to get arrival_time and burst_time for each process
            int arrival_time = arrival_times.get(i); // Replace with actual method to get arrival time
            int burst_time = burst_times.get(i); // Replace with actual method to get burst time
